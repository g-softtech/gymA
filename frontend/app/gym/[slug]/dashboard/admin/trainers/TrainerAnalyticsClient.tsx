    ...
    <div className="space-y-6 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-500">Total Sessions Booked</p>
            <h3 className="text-3xl font-bold text-gray-900 mt-1">{data.totalSessions}</h3>
          </div>
          <div className="p-3 bg-indigo-50 text-indigo-600 rounded-lg text-2xl">🏋️</div>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-500">Total Active Clients</p>
            <h3 className="text-3xl font-bold text-gray-900 mt-1">{data.totalActiveClients}</h3>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <h3 className="text-base font-bold text-gray-900 mb-1">Sessions by Trainer</h3>
        <p className="text-xs text-gray-400 mb-6">Total sessions booked per trainer (excluding cancellations)</p>
        <div className="h-64">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="text-base font-bold text-gray-900">Trainer Utilization</h3>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {data.trainers.map((t: TrainerStat) => (
                <tr key={t.trainerId} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 font-medium text-gray-900">{t.trainerName || "Unknown"}</td>
                  <td className="px-6 py-4 text-gray-700">{t.totalSessions}</td>
                  <td className="px-6 py-4 text-gray-700">{t.activeClients}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-semibold text-gray-700">{t.utilizationPct}%</span>
                    </div>
                  </td>
                </tr>
              ))}
